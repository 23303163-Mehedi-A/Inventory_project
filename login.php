...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$email || !$pass) {
        ...
        if ($cust && password_verify($pass, $cust['password'])) {
            $_SESSION['customer_id']=$cust['id']; $_SESSION['customer_name']=$cust['name'];
            session_regenerate_id(true);
...
.fan-slide img {
  ...
  display: block;
  filter: blur(1.5px) brightness(0.65) saturate(0.8) hue-rotate(8deg);
}

/* Ken Burns — gentle zoom + drift, each slide different direction */
...
/* Navy curtain — lighter at centre, darker at edges */
.fan-curtain {
  position: absolute;
  inset: 0;
  z-index: 1;
  background: radial-gradient(
    ...
    rgba(5, 16, 50, 0.22) 0%,
    rgba(4, 14, 44, 0.55) 55%,
    rgba(3,  9, 33, 0.90) 100%
  );
  box-shadow: inset 0 0 90px 35px rgba(2, 8, 30, 0.65);
}
...
<div class="auth-page">
  ...
  <div class="auth-left">
    ...
    <div class="fan-stage" aria-hidden="true">
      ...
      <!-- 1. White stand fan in room — by Delaney Van -->
      <div class="fan-slide active">
        <img src="https://images.unsplash.com/photo-1601084195907-44baaa49dabd?w=900&q=85&fit=crop"
             alt="white stand fan" loading="eager"/>
      </div>

      <!-- 2. White desk fan — by Ronan Furuta -->
      <div class="fan-slide">
        <img src="https://images.unsplash.com/photo-1565151443833-29bf2ba5dd8d?w=900&q=85&fit=crop"
             alt="white desk fan" loading="lazy"/>
      </div>

      <!-- 3. White 5-blade ceiling fan — by Kalyanaraman S -->
      <div class="fan-slide">
        <img src="https://images.unsplash.com/photo-1609519479841-5fd3b2884e17?w=900&q=85&fit=crop"
             alt="ceiling fan" loading="lazy"/>
      </div>

      <!-- 4. Fan mounted on wall in a room — by Luke Thornton -->
      <div class="fan-slide">
        <img src="https://plus.unsplash.com/premium_photo-1677172408819-a493426f6e10?w=900&q=85&fit=crop"
             alt="wall mounted fan" loading="lazy"/>
      </div>

      <!-- 5. Computer / cooler fan on white table — by Behnam Norouzi -->
      <div class="fan-slide">
        <img src="https://plus.unsplash.com/premium_photo-1721133263972-3b12e3ba6420?w=900&q=85&fit=crop"
             alt="cooler fan" loading="lazy"/>
      </div>

    </div><!-- /fan-stage -->

    <!-- Navy curtain overlay -->
...
(function () {
  ...
  var cur     = 0;
  var DISPLAY = 2000; // ms — change this one number to adjust speed for all
